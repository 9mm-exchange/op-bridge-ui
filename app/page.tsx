...
const navLinks = [
  { href: "https://testnet-explorer.optipulse.io", label: "Explorer", target: "_blank" },
...
export default function Bridge() {
  ...
  const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (activeTab === "deposit" && parseFloat(value) > depositCap) {
      ...
      setPopupDescription(
        `We are currently capping deposits at ${depositCap}  ${NEXT_PUBLIC_NATIVE_TOKEN_SYMBOL}, as ${NEXT_PUBLIC_L2_CHAIN_NAME} may be discontinued at any time. Only deposit funds you are willing to lose.`
      );
      return;
    }
    setAmount(value);
    ...
    <div className="min-h-screen bg-background pt-16">
      ...
      <main className="container mx-auto p-4 max-w-md flex flex-col space-y-8">
        ...
        <Tabs defaultValue="deposit" className="w-full">
          ...
          <TabsContent value="deposit">
            <p className="text-sm text-muted-foreground">
              Deposit {NEXT_PUBLIC_NATIVE_TOKEN_SYMBOL} from {NEXT_PUBLIC_L1_CHAIN_NAME} to{" "}
              {NEXT_PUBLIC_L2_CHAIN_NAME}.
            </p>
          </TabsContent>
          <TabsContent value="withdraw">
            <p className="text-sm text-muted-foreground">
              Withdraw {NEXT_PUBLIC_NATIVE_TOKEN_SYMBOL} from {NEXT_PUBLIC_L2_CHAIN_NAME} to{" "}
              {NEXT_PUBLIC_L1_CHAIN_NAME}.
            </p>
          </TabsContent>
        </Tabs>
        ...
        <div className="space-y-3">
          <Label htmlFor="amount">Amount ({NEXT_PUBLIC_NATIVE_TOKEN_SYMBOL})</Label>
          <Input
            autoFocus
            id="amount"
            placeholder="0.0"
          ...
          {isConnected && (
            <div className={`flex flex-col space-y-1`}>
              <p className="text-sm text-muted-foreground">
                {activeTab === "deposit" ? NEXT_PUBLIC_L1_CHAIN_NAME : NEXT_PUBLIC_L2_CHAIN_NAME} Balance:{" "}
                {activeTab === "deposit"
              ...
              <p className="text-sm text-muted-foreground">
                {activeTab === "withdraw" ?  NEXT_PUBLIC_L1_CHAIN_NAME : NEXT_PUBLIC_L2_CHAIN_NAME} Balance:{" "}
                {activeTab === "withdraw"
                  ? l1Balance?.formatted ?? "0"
                  : l2Balance?.formatted ?? "0"}{" "}
                 {NEXT_PUBLIC_NATIVE_TOKEN_SYMBOL}
        ...
        <Dialog open={isConfirmationOpen} onOpenChange={setIsConfirmationOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                You are about to {activeTab} {amount}  {NEXT_PUBLIC_NATIVE_TOKEN_SYMBOL}{" "}
